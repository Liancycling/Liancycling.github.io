...
function initDashboard() {
    ...
    aiFileInput.addEventListener('change', (e) => {
      ...
      if (file) {
        ...
        if (file.size > maxBytes) {
          alert(`⚠️ 您選擇的檔案「${file.name}」大小約 ${(file.size / 1024 / 1024).toFixed(2)}MB，已超過 20MB 的限制。\n請改用上方的「雲端連結」進行提交！`);
          aiFileInput.value = '';
          aiFileNameDisplay.textContent = "超過 20MB 限制，請清除";
        }
      } else {
  ...
  if (aiFileUploadBtn) {
    aiFileUploadBtn.addEventListener('click', async () => {
      ...
      const maxBytes = 20 * 1024 * 1024; // 20MB
      if (file.size > maxBytes) {
        alert("⚠️ 檔案大小超過 20MB，無法直接上傳，請使用上方的雲端連結提交！");
        return;
      }
